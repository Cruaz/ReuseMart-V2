...
<div class="">
			...
			<div class="card-body p-4">
				<div class="d-flex justify-content-between">
					<div class="d-flex align-items-center">
						<h5 class="card-title h2">Alamat</h5>
					</div>
				...
				<form id="submitFormInsert">
					...
					<div class="modal-body">
						...
						<div class="mb-3">
							...
							<input type="text" class="form-control" name="label_alamat">
						</div>
						<div class="mb-3">
							<label>Deskripsi Alamat</label>
							<input type="text" class="form-control" name="deskripsi_alamat">
						</div>
						<div class="mb-3">
							<label>Is Default</label>
							<input type="number" class="form-control" name="is_default">
						</div>
					</div>
    ...
    <div class="modal fade" id="modalUpdate" tabindex="-1" aria-labelledby="modalUpdateLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">Update Alamat</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                </div>
                <form id="submitFormUpdate">
                    @csrf
                    <input type="hidden" name="id_alamat" id="update-id">
                    <div class="modal-body">
                        <div class="mb-3">
                            <label>ID Pembeli</label>
                            <input type="text" class="form-control" name="id_pembeli" id="update-id_pembeli">
                        </div>
                        <div class="mb-3">
                            <label>Label Alamat</label>
                            <input type="text" class="form-control" name="label_alamat" id="update-label_alamat">
                        </div>
						<div class="mb-3">
                            <label>Deskripsi Alamat</label>
                            <input type="text" class="form-control" name="deskripsi_alamat" id="update-deskripsi_alamat">
                        </div>
                        <div class="mb-3">
                           <label>Is Default</label>
							<input type="number" class="form-control" name="is_default">
    ...
    <div class="modal fade" id="modalDelete" tabindex="-1" aria-labelledby="modalDeleteLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <p>Apakah kamu yakin ingin menghapus Alamat ini?</p>
...
<script>
	...
	function fetchData(page = 1, itemsPerPage = perPage) {
		...
		axios.get(`/api/alamat?page=${page}&per_page=${itemsPerPage}&search=${encodeURIComponent(search)}`, {
			...
			paginatedData.data.forEach(org => {
				tableBody.innerHTML += `
					<tr>
						...
						<td>${org.label_alamat}</td>
                        <td>${org.deskripsi_alamat}</td>
						<td>${org.is_default}</td>
						<td>
							<div class="dropdown">
	...
	document.getElementById('submitFormInsert').addEventListener('submit', function(e) {
		...
		axios.post('/api/alamat', formData, {
			headers: {
    ...
    document.addEventListener('click', function(e) {
        if (e.target.classList.contains('update-btn')) {
            ...
            document.getElementById('update-id_pembeli').value = cells[1].textContent;
            document.getElementById('update-label_alamat').value = cells[2].textContent;
			document.getElementById('update-deskripsi_alamat').value = cells[3].textContent;
        }
    ...
    document.getElementById('submitFormUpdate').addEventListener('submit', function(e) {
        ...
        axios.post(`/api/alamat/${id}`, formData, {
            headers: {
                'Authorization': `Bearer ${token}`,
                'Content-Type': 'multipart/form-data'
            }
